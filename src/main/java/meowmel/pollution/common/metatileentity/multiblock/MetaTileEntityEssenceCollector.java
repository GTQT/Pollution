    ...
    private void handleFocusedMode(double finalSpeedPerTick) {
        ...
        switch (translationKey) {
            case "tile.crystal_aer":
                addPollutionMaterial(InfusedAir, finalSpeedPerTick);
                break;
            case "tile.crystal_ignis":
                addPollutionMaterial(InfusedFire, finalSpeedPerTick);
                break;
            case "tile.crystal_terra":
                addPollutionMaterial(InfusedEarth, finalSpeedPerTick);
                break;
            case "tile.crystal_aqua":
                addPollutionMaterial(InfusedWater, finalSpeedPerTick);
                break;
            case "tile.crystal_ordo":
                addPollutionMaterial(InfusedOrder, finalSpeedPerTick);
                break;
            case "tile.crystal_perditio":
                addPollutionMaterial(InfusedEntropy, finalSpeedPerTick);
                break;
            default:
    ...
    private void handleNormalMode(int finalSpeedPerTick) {
        if (this.outputFluidInventory != null && this.outputFluidInventory.getTanks() > 0) {
            List<FluidStack> fluids = Arrays.asList(
                    InfusedAir.getFluid(finalSpeedPerTick),
                    PollutionMaterials.InfusedFire.getFluid(finalSpeedPerTick),
                    PollutionMaterials.InfusedEarth.getFluid(finalSpeedPerTick),
                    PollutionMaterials.InfusedWater.getFluid(finalSpeedPerTick),
                    PollutionMaterials.InfusedOrder.getFluid(finalSpeedPerTick),
                    PollutionMaterials.InfusedEntropy.getFluid(finalSpeedPerTick)
            );
