...
import meowmel.pollution.api.utils.POTeleporter;
import meowmel.pollution.api.utils.POWorld;
...
public class BlockPOPortal extends BlockBreakable {
    ...
    public BlockPOPortal() {
        ...
        this.setRegistryName("pollution", "Portal");
        this.setHardness(-1F);
    ...
    public boolean tryToCreatePortal(World world, BlockPos pos, EntityItem catalyst, @Nullable EntityPlayer player) {
        ...
        if (canFormPortal(state) && world.getBlockState(pos.down()).isFullCube()) {
            ...
            if (recursivelyValidatePortal(world, pos, blocksChecked, size, state) && size.intValue() >= MIN_PORTAL_SIZE) {
                ...
                if (true) {
                    POTeleporter teleporter = POTeleporter.getTeleporterForDim(catalyst.getServer(), getDestination(catalyst));
                    boolean checkProgression = POWorld.isProgressionEnforced(catalyst.world);
