...
public class MetaTileEntityNodeFusionReactor extends MultiMapMultiblockController implements ICleanVis, IFastRenderMetaTileEntity, IBloomEffect {
	...
	private List<Integer> readNodeInfo(){
		...
		for (var i = 0; i < this.getInputInventory().getSlots(); ++i) {
			if (!this.inputInventory.getStackInSlot(i).isEmpty()) {
				...
				if (blackAmount < 0) {blackAmount = 0;}
	...
	private IBlockState getCasingState3() {
		...
		} else return this.tier == 7 ? PollutionMetaBlocks.HYPER.getState(POHyper.HyperType.HYPER_2_CASING) : PollutionMetaBlocks.HYPER.getState(POHyper.HyperType.HYPER_3_CASING);
	}
